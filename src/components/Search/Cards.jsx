...
const Cards = () => {
  ...
  const handleBook = (title) => {
    ...
    // const tomorrowFormatted = tomorrow.toISOString().substring(0, 10);const currentDate = new Date();
    ...
    const queryString = `?q=${encodeURIComponent(title)}&checkIn=${encodeURIComponent(checkInDate)}&checkOut=${encodeURIComponent(checkOutDate)}&rooms=${encodeURIComponent(rooms)}`;
